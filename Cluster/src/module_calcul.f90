...
SUBROUTINE apply_kernel_k_means(proc_id,nb_clusters_max,nb_clusters_opt,partitioned_data,clust_param)
   ...
   INCLUDE 'mpif.h'
    ...
    DOUBLE PRECISION :: listnorm (partitioned_data%nb, partitioned_data%nb_clusters)
...
SUBROUTINE mean_shift(proc_id,nb_clusters_max,nb_clusters_opt,partitioned_data,bandwidth)
   ...
   INCLUDE 'mpif.h'
	...
	INTEGER :: clusterVotes(partitioned_data%nb_clusters,partitioned_data%nb)					!number of votes for each point for each cluster
    INTEGER :: i
    INTEGER :: j
    INTEGER :: num
