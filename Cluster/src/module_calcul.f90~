...
SUBROUTINE apply_kernel_k_means(numproc,nblimit,nbideal,dataw,ker_param)
    IMPLICIT NONE
  ...
  ! INCLUDE 'mpif.h'
   ...
   ! LOGICAL :: found
    ...
    IF (ker_param%kernelfunindex==0 .AND. (ker_param%gam <0.0 .OR. ker_param%delta<0.0) )  THEN
       WRITE ( *, '(a)' ) ' '
       WRITE ( *, '(a)' ) 'KERNELKMEANS_01 - Fatal error!'
    ...
    IF (ker_param%kernelfunindex==0) THEN
        Ker=poly_kernel( dataw, ker_param%gam, ker_param%delta)
    ELSEIF (ker_param%kernelfunindex==1) THEN
        Ker=gaussian_kernel(dataw, ker_param%sigma)
    ENDIF
...
!!! boucle            
  ...
  SUBROUTINE apply_spectral_clustering(numproc, nb_clusters_max, nbideal, partitioned_data, sigma,ker_param)
    IMPLICIT NONE
    INCLUDE 'mpif.h'
    !###########################################
    ! DECLARATIONS
